...
try {
  ...
  $voteCountsStmt = $pdo->prepare("
    SELECT
      COALESCE(SUM(value), 0) AS net,
      COUNT(*) FILTER (WHERE value = -1) AS downvotes,
      COUNT(*) AS total
    FROM translation_votes WHERE suggestion_id = ?
  ");
  $voteCountsStmt->execute([$suggestionId]);
  $voteCounts = $voteCountsStmt->fetch();
  $netScore = (int) $voteCounts['net'];
  $downvotes = (int) $voteCounts['downvotes'];
  $totalVotes = (int) $voteCounts['total'];

  $acceptThreshold = (int) get_config($pdo, 'auto_accept_threshold', '5');
  $declineThreshold = (int) get_config($pdo, 'auto_decline_threshold', '-3');

  // Suggestions for a key/locale that already has an accepted translation
  // don't get a time-based fallback (see resolveExpiredPending in
  // tools/scripts/export-translations.js) - a bad-ratio decline is their only
  // route out of "pending" besides crossing the accept threshold above, so it
  // has to be checked instantly here rather than by a daily batch job.
  $minVotesForRatioDecline = (int) get_config($pdo, 'min_votes_for_ratio_decline', '5');
  $declineRatioThreshold = (float) get_config($pdo, 'decline_ratio_threshold', '0.7');
  $ratioDecline = $totalVotes >= $minVotesForRatioDecline && ($downvotes / $totalVotes) >= $declineRatioThreshold;
  ...
  if ($netScore >= $acceptThreshold) {
    ...
    $newStatus = 'accepted';
  } elseif ($netScore <= $declineThreshold || $ratioDecline) {
