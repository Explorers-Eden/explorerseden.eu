...
try {
  ...
  $stmt = $pdo->prepare('
    SELECT
      ...
      EXISTS (
        SELECT 1 FROM translation_suggestions other
        WHERE other.translation_key_id = ts.translation_key_id
          AND other.locale_code = ts.locale_code
  ...
  $result = array_map(fn($r) => [
    ...
    // A suggestion is a "follow-up" once its key/locale already has an
    // accepted translation - those never get a time-based fallback (see
    // resolveExpiredPending in tools/scripts/export-translations.js), so the
    // frontend shouldn't show a time-remaining countdown for them.
    'hasAcceptedTranslation' => (bool) $r['has_accepted'],
