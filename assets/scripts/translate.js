(function () {
  ...
  // Shows a string exactly as it appears in the source lang JSON file - real
  // newline characters as the literal two-character escape "\n" rather than
  // as a rendered line break. Translators need to see and reproduce this
  // literal escape sequence (like the §l/§r formatting codes) verbatim in
  // their suggestion, since that's what actually gets written back to the
  // data pack's lang file - a rendered blank line hides how many \n's are
  // really there.
  ...
  function keyRowHtml(key) {
    ...
    const currentHtml = key.accepted
      ? `${escRaw(key.accepted.body)}${key.accepted.possiblyOutdated ? '<i class="bi bi-exclamation-triangle-fill translate-row__outdated" title="The English source text has changed since this was submitted"></i>' : ''}`
    ...
    return `
      <tr class="translate-row ${staged ? 'translate-row--staged' : ''}" data-key-id="${key.id}">
        <td class="translate-row__source">
          <span class="translate-row__text">${escRaw(key.sourceText)}</span>
        ...
        <td class="translate-row__actions">
          <form class="translate-suggest-form" data-key-id="${key.id}">
            <textarea name="body" placeholder="Suggest a translation…" maxlength="2000" rows="1" ${me.loggedIn ? '' : 'disabled'}>${esc(staged)}</textarea>
          </form>
        </td>
  ...
  function modalSuggestionHtml(suggestion, isFollowUp) {
      ...
      return `
        <div class="translate-suggestion translate-suggestion--accepted">
          ...
          <div class="translate-suggestion__body">
            <p class="translate-suggestion__text">${escRaw(suggestion.body)}</p>
    ...
    const editBtn = suggestion.isMine
      ? `<button type="button" class="translate-suggestion__edit" data-suggestion-id="${suggestion.id}" title="Edit your suggestion"><i class="bi bi-pencil" aria-hidden="true"></i></button>`
      : '';
    ...
    return `
      <div class="translate-suggestion" data-suggestion-id="${suggestion.id}">
        ...
        <div class="translate-suggestion__body">
          <p class="translate-suggestion__text">${escRaw(suggestion.body)}</p>
          <p class="translate-suggestion__meta">by ${esc(suggestion.author)} · ${relativeTimeLabel(suggestion.createdAt)}${isFollowUp ? '' : ` · ${timeRemainingLabel(suggestion.createdAt)}`}${votesNeededLabel(suggestion.netScore) ? ` · ${votesNeededLabel(suggestion.netScore)}` : ''}</p>
        </div>
        ${editBtn}
        ${deleteBtn}
      </div>
  ...
  async function saveSuggestionEdit(suggestionId, body) {
    await fetchJson('/translate/api/edit-suggestion.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': me.csrfToken || '' },
  ...
  // Swaps a rendered suggestion body in place for a textarea (needed so
  // multi-line bodies stay editable, not just single lines) with Save/Cancel
  // controls. `onSave` does the actual persist + reload; this only owns the
  // DOM swap and restoring the original markup on cancel.
  function enterSuggestionEditMode(bodyEl, currentBody, onSave) {
    const original = bodyEl.innerHTML;
    bodyEl.innerHTML = `
      <form class="translate-edit-form">
        <textarea class="translate-edit-form__input" maxlength="2000" required>${esc(currentBody)}</textarea>
        <div class="translate-edit-form__actions">
          <button type="submit" class="translate-edit-form__save">Save</button>
          <button type="button" class="translate-edit-form__cancel">Cancel</button>
        </div>
      </form>
    `;
    const form = bodyEl.querySelector('.translate-edit-form');
    const textarea = form.querySelector('textarea');
    textarea.focus();
    textarea.setSelectionRange(textarea.value.length, textarea.value.length);

    form.querySelector('.translate-edit-form__cancel').addEventListener('click', () => {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (!newBody) return;
      const saveBtn = form.querySelector('.translate-edit-form__save');
      saveBtn.disabled = true;
      try {
        await onSave(newBody);
      } catch (err) {
        alert(err.message);
  ...
  function renderModalContent(key) {
    ...
    modalSource.textContent = String(key.sourceText ?? '').replace(/\n/g, '\\n');
    const all = key.accepted ? [key.accepted, ...key.pendingSuggestions] : key.pendingSuggestions;
    const isFollowUp = Boolean(key.accepted);
    modalList.innerHTML = all.length
      ? all.map((s) => modalSuggestionHtml(s, isFollowUp)).join('')
  ...
  function attachDetailHandlers() {
    ...
    stringsBox.querySelectorAll('.translate-suggest-form').forEach((form) => {
      ...
      textarea.addEventListener('input', () => {
        stageSuggestion(keyId, textarea.value.trim(), row);
      });

      // No submit button on this form - nothing implicitly submits it, but
      // guard against a stray Enter-triggered submit anyway.
  ...
  function attachModalHandlers() {
    ...
    modalList.querySelectorAll('.translate-suggestion__edit').forEach((btn) => {
      btn.addEventListener('click', () => {
        const suggestionId = Number(btn.dataset.suggestionId);
        const suggestionEl = btn.closest('.translate-suggestion');
        const bodyEl = suggestionEl.querySelector('.translate-suggestion__body');
        const key = allKeys.find((k) => k.id === openModalKeyId);
        const suggestion = key && [key.accepted, ...key.pendingSuggestions].find((s) => s && s.id === suggestionId);
        if (!suggestion) return;
        enterSuggestionEditMode(bodyEl, suggestion.body, async (newBody) => {
          await saveSuggestionEdit(suggestionId, newBody);
          const keyId = openModalKeyId;
          await loadStrings();
          openModal(keyId);
        });
  ...
  function suggestionRowHtml(s, { showAuthor, canDeleteFn, canEditFn, deleteEndpointFn, interactiveVotes, showTimeRemaining }) {
    const canDelete = s.status === 'pending' && canDeleteFn(s);
    const canEdit = s.status === 'pending' && canEditFn(s);
    const editBtn = canEdit
      ? `<button type="button" class="translate-suggestion__edit" data-suggestion-id="${s.id}" title="Edit this suggestion"><i class="bi bi-pencil" aria-hidden="true"></i></button>`
      : '';
    ...
    // A suggestion whose key/locale already has an accepted translation is a
    // "follow-up" - it never gets a time-based fallback (see
    // resolveExpiredPending in tools/scripts/export-translations.js), so no
    // countdown is shown for it even though it's still pending.
    const timeRemainingHtml = (s.status === 'pending' && showTimeRemaining && !s.hasAcceptedTranslation)
    ...
    return `
      <tr class="translate-row" data-suggestion-id="${s.id}">
        <td class="translate-suggestions-table__source">
          <span class="translate-row__text">${escRaw(s.sourceText)}</span>
          <code class="translate-row__path">${esc(s.datapackName)} · ${esc(s.localeName)}</code>
        </td>
        <td class="translate-suggestions-table__body">${escRaw(s.body)}${timeRemainingHtml}${votesNeededHtml}</td>
        ${authorCell}
  ...
  function renderSuggestionsTable(container, list, { showAuthor, canDeleteFn, canEditFn, deleteEndpointFn, interactiveVotes = false, showTimeRemaining = false, onReload }) {
    const headerCols = [
      ...
      <div class="table-card translate-table-card">
        <div class="table-scroll">
          <table class="translate-suggestions-table">
            ...
            <tbody>${list.map((s) => suggestionRowHtml(s, { showAuthor, canDeleteFn, canEditFn, deleteEndpointFn, interactiveVotes, showTimeRemaining })).join('')}</tbody>
          </table>
        </div>
      </div>
    `;
    ...
    container.querySelectorAll('.translate-suggestion__edit').forEach((btn) => {
      btn.addEventListener('click', () => {
        const suggestionId = Number(btn.dataset.suggestionId);
        const bodyEl = btn.closest('tr').querySelector('.translate-suggestions-table__body');
