(function () {
  ...
  // break. See translate.js's escRaw for why: translators must reproduce
  // this literal sequence verbatim in their suggestion.
  ...
  function suggestionRowHtml(s) {
    ...
    const editBtn = s.isMine
      ? `<button type="button" class="translate-suggestion__edit" data-suggestion-id="${s.id}" title="Edit your suggestion"><i class="bi bi-pencil" aria-hidden="true"></i></button>`
      : '';
    ...
    return `
      <tr class="translate-row" data-suggestion-id="${s.id}">
        ...
        <td class="translate-suggestions-table__body">
          ${escRaw(s.body)}
          ${s.hasAcceptedTranslation ? '' : `<span class="translate-row__time-left">${timeRemainingLabel(s.createdAt)}</span>`}
  ...
  function renderTable() {
    ...
    list.querySelectorAll('.translate-suggestion__edit').forEach((btn) => {
