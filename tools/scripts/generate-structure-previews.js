...
function getBlockNameFromPaletteEntry(entry) {
  // MC 26.3+ (DataVersion 5021+) renamed the structure NBT palette tag from
  // Name/Properties to id/properties.
  return entry?.Name ?? entry?.name ?? entry?.id ?? entry?.Id ?? null;
}
...
// Structure processor lists (worldgen/processor_list/*.json) apply per-block
// transforms (e.g. random mossy/decay replacement) when a jigsaw pool element
// places a piece. Only `minecraft:rule` is implemented — it is the only
// processor type that changes which blocks are rendered; others (protected_blocks,
// capped, gravity, block_rot, jigsaw_replacement, ...) affect worldgen placement
// behavior, not the piece's own block content, so previews can ignore them.
function loadProcessorList(processorsId) {
  const normalized = normalizeResourceLocationForCompare(processorsId);
  if (!normalized || normalized === "minecraft:empty") return [];

  const [namespace, procPath] = splitResourceLocation(normalized);
  const json = readJsonIfExists(path.join(inputRoot, namespace, "worldgen", "processor_list", `${procPath}.json`));
  const list = json?.processors ?? json;
  return Array.isArray(list) ? list : [];
}

function outputStateFromRule(outputState) {
  if (!outputState) return null;
  if (typeof outputState === "string") return parseBlockStateString(outputState);
  if (typeof outputState === "object") {
    const name = outputState.Name ?? outputState.name ?? outputState.id;
    const properties = outputState.Properties ?? outputState.properties ?? {};
    return name ? { name: normalizeResourceLocationForCompare(name), properties } : null;
  }
  return null;
}

function blockStateSpecMatches(blockStateSpec, blockName, properties) {
  if (!blockStateSpec) return false;
  const name = blockStateSpec.Name ?? blockStateSpec.name;
  if (name && normalizeResourceLocationForCompare(name) !== blockName) return false;
  const wantedProps = blockStateSpec.Properties ?? blockStateSpec.properties ?? {};
  for (const [key, value] of Object.entries(wantedProps)) {
    if (String(properties?.[key]) !== String(value)) return false;
  }
  return true;
}

// Covers the predicate types actually used by processor lists in the wild
// (always_true, block_match, random_block_match, and their blockstate
// siblings). Anything unrecognized (e.g. tag_match, linear_pos) is treated as
// non-matching so an unsupported predicate never misapplies a rule.
function ruleLocationOrInputMatches(predicate, blockName, properties, seedText) {
  if (!predicate) return true;
  const type = predicate.predicate_type ?? predicate.type ?? "minecraft:always_true";

  switch (type) {
    case "minecraft:always_true":
      return true;
    case "minecraft:block_match":
      return normalizeResourceLocationForCompare(predicate.block) === blockName;
    case "minecraft:random_block_match": {
      if (normalizeResourceLocationForCompare(predicate.block) !== blockName) return false;
      return seededRandom(`${seedText}|prob`) < Number(predicate.probability ?? 1);
    }
    case "minecraft:blockstate_match":
      return blockStateSpecMatches(predicate.block_state, blockName, properties);
    case "minecraft:random_blockstate_match": {
      if (!blockStateSpecMatches(predicate.block_state, blockName, properties)) return false;
      return seededRandom(`${seedText}|prob`) < Number(predicate.probability ?? 1);
    }
    default:
      return false;
...
function applyStructureProcessors(blocks, processorsId, seedText) {
  const processors = loadProcessorList(processorsId);
  if (processors.length === 0) return blocks;

  let current = blocks;
  for (let pIndex = 0; pIndex < processors.length; pIndex++) {
    const processor = processors[pIndex];
    const type = processor?.processor_type ?? processor?.type;
    if (type !== "minecraft:rule") continue;

    const rules = Array.isArray(processor.rules) ? processor.rules : [];
    ...
    for (const block of current) {
      ...
      for (let rIndex = 0; rIndex < rules.length; rIndex++) {
        ...
        const ruleSeed = `${seedText}|proc${pIndex}|rule${rIndex}|${block.x},${block.y},${block.z}`;

        if (!ruleLocationOrInputMatches(rule.location_predicate, block.name, block.properties, `${ruleSeed}|loc`)) continue;
        if (!ruleLocationOrInputMatches(rule.input_predicate, block.name, block.properties, `${ruleSeed}|in`)) continue;

        const output = outputStateFromRule(rule.output_state);
        if (output) replaced = { ...block, name: output.name, properties: { ...output.properties } };
        break;
      }

      const result = replaced ?? block;
      if (!IGNORED_BLOCKS.has(result.name)) next.push(result);
    }
    current = next;
  }
...
function transformStructureBlocks(structure, offset, quarterTurns = 0, processorsId = null, seedText = previewSeed) {
  const size = getStructureSize(structure);
  const rawBlocks = collectBlocksFromStructure(structure);
  // Processors apply to the piece's own local-space blocks, before rotation/offset
  // into the assembled preview — matching how the structure template itself is
  // processed at placement time, independent of where it lands.
  const processedBlocks = processorsId ? applyStructureProcessors(rawBlocks, processorsId, seedText) : rawBlocks;
...
async function chooseStructuresFromTemplatePool(poolId, seenPools = new Set(), seedText = previewSeed) {
  ...
  for (const choice of ordered) {
    ...
    candidates.push({
      ...
      processors: choice.element?.processors
    });
  }
...
async function assembleJigsawFromStartFile(startStructureFile, options = {}) {
  ...
  const queue = [{
    ...
    processors: options.startProcessors ?? null
  }];
  ...
  while (queue.length > 0) {
    ...
    const size = getStructureSize(structure);
    const transformedBlocks = transformStructureBlocks(
      structure,
      item.offset,
      item.quarterTurns,
      item.processors,
      `${previewSeed}|${item.structureFile}`
    );
      ...
      for (const childChoice of childChoices) {
        ...
        for (const childConnector of childConnectors) {
          ...
          queue.push({
            ...
            processors: childChoice.processors
          });
          placedChild = true;
          break;
        }
...
async function assembleJigsawStructureFromPool(startPool, options = {}) {
  ...
  return assembleJigsawFromStartFile(start.structureFile, { ...options, startProcessors: start.processors });
}
...
async function collectStructureFilesForWorldgenStructure(worldgenFile) {
  ...
  for (const choice of getTemplatePoolChoices(poolJson)) {
    for (const location of choice.locations ?? (choice.location ? [choice.location] : [])) {
      ...
      if (!fs.existsSync(structureFile)) continue;
      seen.add(location);
      startLocations.push({ location, structureFile, processors: choice.element?.processors });
    }
  }
  ...
  if (startLocations.length === 1) {
    const assembled = await assembleJigsawFromStartFile(startLocations[0].structureFile, {
      ...constraints,
      startProcessors: startLocations[0].processors
    });
  ...
  for (const entry of startLocations) {
    const assembled = await assembleJigsawFromStartFile(entry.structureFile, {
      ...constraints,
      startProcessors: entry.processors
    });
