...
//
// The time limit only applies to "first-time" suggestions - ones for a fresh
// key/locale that has no accepted translation yet - where a suggestion with
// zero downvotes wins on time alone (no need to chase 5 upvotes for a
// language nobody's looked at yet). A key/locale that already HAS an
// accepted translation only ever gets "follow-up" suggestions from here on,
// and those have no time limit at all: they're skipped entirely below and
// stay pending indefinitely until votes resolve them one way or the other -
// accepted by crossing auto_accept_threshold, or declined by crossing
// auto_decline_threshold or the bad-ratio check (both handled instantly in
// vote.php, since there's no longer a daily fallback to catch them).
//
...
async function resolveExpiredPending(client) {
  ...
  let accepted = 0;
  let declined = 0;
  let skippedFollowUpGroups = 0;

  for (const [groupKey, members] of groups) {
    // Follow-up suggestions (the key/locale already has an accepted
    // translation) have no time limit at all - leave them pending untouched;
    // only vote.php's instant accept/decline checks resolve these.
    if (hasExistingTranslation.has(groupKey)) {
      skippedFollowUpGroups++;
      continue;
    }

    await client.query('BEGIN');
    try {
      const eligible = members.filter(m => m.downvotes === 0);
  ...
  console.log(`Time-based fallback: ${accepted} accepted, ${declined} declined (of ${expired.rowCount} expired-pending across ${groups.size} key/locale group(s), ${skippedFollowUpGroups} of which were follow-up groups left pending for votes to resolve).`);
}
