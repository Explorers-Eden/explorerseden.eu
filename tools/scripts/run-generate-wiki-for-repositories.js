...
function main() {
  ...
  if (shouldRun('enchantments')) {
    // update-enchantments-json.py reads-and-merges into this file once per
    // repo (it's invoked once per repo below, each call folding that repo's
    // enchantments into whatever's already there) - it errors if the file
    // doesn't exist yet. It used to always exist because it was committed to
    // git; now that it's published to Postgres instead (see
    // tools/scripts/publish-generated-data.js) and no longer committed, a
    // fresh checkout has nothing there, so it needs seeding here instead.
    const enchantmentsJsonPath = path.join(workflowRoot, 'enchantments', 'data', 'enchantments.json');
    fs.mkdirSync(path.dirname(enchantmentsJsonPath), { recursive: true });
    if (!exists(enchantmentsJsonPath)) {
      fs.writeFileSync(enchantmentsJsonPath, '[]\n', 'utf8');
    }
  }
