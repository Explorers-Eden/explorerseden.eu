...
// MC 26.3+ (DataVersion 5021+) renamed loot table entry/pool function lists
// from "functions" to "modifier". Support both so older and newer datapacks parse.
function getEntryFunctions(entry) {
  // "modifier" may be a single object (shorthand) or an array.
  const raw = entry?.functions ?? entry?.modifier ?? [];
...
function getEnchantedBookLabel(entry, details) {
  const randomlyFn = getEntryFunctions(entry).find(f => getFunctionId(f) === "minecraft:enchant_randomly");
  const withLevelsFn = getEntryFunctions(entry).find(f => getFunctionId(f) === "minecraft:enchant_with_levels");
...
function resolveEntryFunctions(entry) {
  const resolved = [];
  for (const fn of getEntryFunctions(entry)) {
    if (getFunctionId(fn) === "minecraft:reference") {
...
function getComponentSources(entry) {
  ...
  return [
    entry.components,
    ...resolvedFunctions
      .filter(f => getFunctionId(f) === "minecraft:set_components")
      .map(f => f.components ?? {})
  ];
}
...
function getItemNameComponent(entry) {
  ...
  const resolvedFunctions = resolveEntryFunctions(entry);
  const nameFn =
    resolvedFunctions.find(f => getFunctionId(f) === "minecraft:set_name") ??
    resolvedFunctions.find(f => getFunctionId(f) === "minecraft:set_custom_name");

  return nameFn?.name;
}

function getStackSize(entry) {
  const fn = getEntryFunctions(entry).find(f => getFunctionId(f) === "minecraft:set_count");
  if (!fn) return "1";
...
function isEnchantedBook(entry) {
  return (
    ...
    getEntryFunctions(entry).some(
      f =>
        getFunctionId(f) === "minecraft:enchant_with_levels" ||
        getFunctionId(f) === "minecraft:enchant_randomly"
    )
  );
}
...
async function getSingleEntryFromLootTable(id, seen = new Set()) {
  ...
  try {
    ...
    for (const pool of json.pools ?? []) {
      entries.push(...flattenRawEntries(pool.entries ?? [], getEntryFunctions(pool)));
    }
...
async function renderMergedPools(pools, details = null) {
  ...
  for (const [poolIndex, pool] of (pools ?? []).entries()) {
    const flattenedEntries = await flattenEntries(pool.entries ?? [], 1, getEntryFunctions(pool), new Set(), details);
