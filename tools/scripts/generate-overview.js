...
// MC 26.3+ (DataVersion 5021+) renamed loot table entry function lists from
// "functions" to "modifier", and each function's discriminator key from
// "function" to "type". Support both old and new so every datapack parses.
  ...
  function getEntryCustomName(entry) {
    ...
    if (!customName) {
      for (const fn of getEntryFunctions(entry)) {
        ...
        if ((fnId === 'minecraft:set_name' || fnId === 'minecraft:set_custom_name') && fn.name) {
