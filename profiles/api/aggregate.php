...
try {
  ...
  foreach (get_all_player_profiles() as $decoded) {
  ...
  // Minimal per-player list (just enough to render a random skin lineup) -
  // deliberately excludes stats/advancements since those are already
  // covered by totals/leaderboards above. uuid lets the client build the
  // homepage's own stand/wave pose render URLs (assets/images/generated/
  // player-skins-home/<uuid>-{stand,wave}.png, from
  // tools/scripts/generate-homepage-skin-poses.js) rather than reusing the
  // single-pose skinIcon render.
  $rosterEntries = [];
  foreach ($players as $p) {
